...
async def generalize_questions(model: str, questions: List[str]) -> str:
    ...
    prompt = f"""Сформулируй один обобщённый ключевой вопрос для группы вопросов:
             {questions_str}
             -----
             Не пиши ничего, кроме обобщённого ключевого вопроса."""

    return (await client.get_completion(prompt, max_tokens=64, rep_penalty=1.0)).strip()
...
async def build_global_plan(model: str, chunks: List[str], max_q_per_chunk: int = 50, sample_per_cluster: int = 20) -> \
List[str]:
