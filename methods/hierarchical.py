from utils import oclient, AsyncList, client
...
async def hierarchical_summary(model, chunks, initial_word_limit=500):
    if not chunks:
        ...
        while i < len(current_level_summaries):
            if i + 2 < len(current_level_summaries):
                ...
                if i + 5 < len(current_level_summaries):
                    temp_summary = await merge_summaries(model, current_level_summaries[i + 3: i + 6],
                                                         current_word_limit, use_context=True,
                                                         previous_summary=temp_summary)
                    i += 6
                else:
                    i += 3
